...
class DocumentModel(Base):
    ...
    file_size = Column(Integer, nullable=False)  # Changed from nullable=True to match schema
    description = Column(Text, nullable=True)
    upload_status = Column(String(50), nullable=False)  # Store enum as string
    embedding_status = Column(String(50), nullable=False, default='pending')  # Add missing field
    created_at = Column(DateTime, nullable=False)
    updated_at = Column(DateTime, nullable=False, default=lambda: datetime.now(timezone.utc).replace(tzinfo=None))  # Add missing field with default
