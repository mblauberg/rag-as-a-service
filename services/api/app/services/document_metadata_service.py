...
class DocumentMetadataService:
    ...
    async def update_embedding_status(
        ...
        if document:
            ...
            await db.commit()
            logger.info(f"Updated embedding status to {status} for document {document_id}")
        else:
            logger.warning(f"Document {document_id} not found when updating embedding status")
    ...
    async def update_upload_status(
        ...
        if document:
            ...
            logger.warning(f"Document {document_id} not found when updating upload status")
    ...
    async def create_chunk_records(
        ...
        logger.info(f"Created {len(chunk_objects)} chunk records for document {document_id}")
