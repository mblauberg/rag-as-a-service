...
async def test_generate_returns_text_response(generation_service):
    ...
    with patch.object(generation_service.client, 'generate', new_callable=AsyncMock) as mock_generate:
        # Mock response with 'text' attribute
        mock_result = type('GenerateResponse', (), {'text': 'Generated text response'})()
