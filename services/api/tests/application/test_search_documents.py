...
async def test_search_documents_success():
    ...
    mock_vector_store.search.assert_called_once_with(query_vector=query_vector, top_k=5)
...
async def test_search_with_query_expansion():
    ...
    mock_embedding_service.generate_embeddings = AsyncMock(
        side_effect=[[[0.1, 0.2, 0.3]], [[0.15, 0.25, 0.35]], [[0.12, 0.22, 0.32]]]
    )
    ...
    mock_vector_store.search = AsyncMock(side_effect=[[chunk1], [chunk2], [chunk3]])
    ...
    mock_query_augmenter.expand.assert_called_once_with(
        "How do I scale Kubernetes?", num_variants=2
    )
...
async def test_search_expansion_with_single_result_set():
    ...
    # Mock query expansion
    mock_query_augmenter.expand = AsyncMock(return_value=["test query"])
...
async def test_search_with_reranking():
    ...
    mock_reranker.rerank.assert_called_once_with(
        query="test query", chunks=initial_chunks, top_k=10
    )
...
async def test_search_with_reranking_disabled():
    ...
    query = SearchQuery(text="test query", top_k=10)
    results = await use_case.execute(query, mode=SearchMode.VECTOR, use_reranking=False)
...
async def test_search_without_reranker():
    ...
    query = SearchQuery(text="test query", top_k=10)
    results = await use_case.execute(query, mode=SearchMode.VECTOR, use_reranking=True)
...
async def test_search_with_reranking_different_top_k():
    ...
    query = SearchQuery(text="test query", top_k=5)
    results = await use_case.execute(
        query, mode=SearchMode.VECTOR, use_reranking=True, rerank_candidates=100
    )
    ...
    mock_reranker.rerank.assert_called_once_with(
        query="test query", chunks=initial_chunks, top_k=5
    )
...
async def test_search_with_reranking_and_expansion():
    ...
    # Mock query expansion
    mock_query_augmenter.expand = AsyncMock(
        return_value=["test query", "query variant 1", "query variant 2"]
    )
    ...
    mock_vector_store.search = AsyncMock(
        side_effect=[create_chunks(0), create_chunks(30), create_chunks(60)]
    )
    ...
    mock_reranker.rerank.assert_called_once_with(
        query="test query", chunks=fused_chunks, top_k=10
    )
