...
class TestDocumentTypeSupport:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("filename,expected_type,should_have_content", SUPPORTED_DOCUMENT_TYPES)
    async def test_complete_workflow_for_document_type(
        ...
        if should_have_content:
            ...
            assert document_data.get("embedding_status") in ["completed", "pending", "processing"]
...
class TestDocumentTypeFailures:
    ...
    async def test_unsupported_file_type_rejection(self, async_client: AsyncClient):
        ...
        for filename, mime_type in unsupported_types:
            ...
            with tempfile.NamedTemporaryFile(mode='wb', suffix=Path(filename).suffix, delete=False) as f:
